...
%put your figures in one place! You need to change this for your computer!
...
\item \verb@echo=FALSE@ means I don't want \TeX to show the R code, note that if you are in the \verb@.Rnw@ file you see \verb@qqnorm (rnorm(100))@, but in the output you don't. That no echoing (pardon my English). The default is to echo.
\item \verb@results=hide@ means don't show the results either. The default is to show all your results.
\item \verb@fig=TRUE@ means yes! show the figure. This is in my mind is a hack figure. Let's fix it up a touch now (we'll center it and add a caption), since we're here.
\end{enumerate}
\end{enumerate}
...
setwd("/Users/Lizzie/Documents/git/demoSweave")
...
@
\\
Let's break down a little of what happened here:
\begin{enumerate}
...
Okay, how about one last trick? We saw that since 1970 harvest dates of the delicious Pinot Noir grapes from Burgundy have been advancing about each year. Let's say we want to discuss the exact value in the text. We want to say that harvest dates of the delicious Pinot Noir grapes from Burgundy have been shifting about \Sexpr{format(coef(aov19)[2], digits=2)} days each year, or \Sexpr{format(coef(aov19)[2]*32, digits=1)} days since 1970. \\
\\
So, all we did was use \verb@Sexpr{}@, and stuck in some R code in line. I also tossed in that \verb@digits@ command again with \verb@format@, if I didn't it would look like this: \ldots delicious Pinot Noir grapes from Burgundy have been shifting about \Sexpr{coef(aov19)[2]} days each year. And, let's be honest, the latter is just not going to impress your friends or reviewers as much as the former.

\section{Now you!}
Now would be a great time for you to muck with this document. First, get it to run on your machine, next try turning on and off things like \verb@echo=FALSE@ or \verb@results=hide@. \\
\\
...
\\
Well, as you may have guessed, we didn't cover everything here. I keep learning more every time I build a giant new Sweave doc and suspect you will too. I have some resources on my webpage and you find many others online. \\
...
\item The file extension for Sweave files with R is \verb@.Rnw@, for Sweave files with S it's \verb@.Snw@. I am not actually sure if this matters in practice, I just thought I would share it.
\end{enumerate}
